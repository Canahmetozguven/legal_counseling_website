...
const downloadImage = (url, imageName) => {
  return new Promise((resolve, reject) => {
    const fileName = `image-${Date.now()}-${Math.round(Math.random() * 1E9)}${path.extname(imageName) || '.jpg'}`;
...
const updateTeamMemberImages = async () => {
  ...
  try {
    ...
    if (!aboutData || !aboutData.teamMembers || aboutData.teamMembers.length === 0) {
    ...
    for (const member of aboutData.teamMembers) {
      if (member.image && member.image.startsWith('https://picsum.photos')) {
        try {
          ...
          const imageName = await downloadImage(member.image, 'team-member.jpg');
    ...
    for (const area of practiceAreas) {
      if (area.image && area.image.startsWith('https://')) {
        try {
          ...
          const imageName = await downloadImage(area.image, 'practice-area.jpg');
